---
description: 'Fixes compilation errors in source or test files. For the trading-bot project, fixes Python syntax errors, import errors, type errors, and pytest failures. Analyzes error messages and applies corrections.'
...
This is a Python 3.10+ project. Common error types to fix:
- **ImportError / ModuleNotFoundError** — missing import, wrong module path
- **TypeError** — wrong type hints or missing type annotation
- **AttributeError** — accessing non-existent attribute on a class
- **`asyncio` errors** — forgetting `await`, using sync function in async context
- **`pandas` errors** — wrong DataFrame operations (e.g., chained assignment)
- **Missing `Candle` fields** — raw broker data not fully transformed to `Candle` model
